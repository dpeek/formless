...
| PSSR-D19 | Use selective Worker-first static-asset routing with client shell and asset exclusions.                                                                           | Published documents need Worker SSR, while generated admin routes, preview routes, and static assets should keep SPA/static serving.                  | `wrangler.jsonc`, `src/worker/routing.ts`, `src/worker/routing.test.ts`                          |
| PSSR-D20 | Use short TTL cache headers for first-slice SSR HTML: 60s success with 300s stale-while-revalidate, 30s 404 with 60s stale-while-revalidate, and no-store errors. | Short TTL improves repeat public reads without requiring active purge; shorter 404 caching limits stale misses after new pages publish.               | `src/worker/site-cache.ts`, `src/worker/site-ssr.test.ts`                                        |
| PSSR-D21 | Keep public Site tree API reads `no-store`.                                                                                                                       | Browser route transitions and preview-adjacent reads should see the authority projection immediately; SSR HTML carries its own short TTL policy.      | `src/worker/site-cache.ts`, `src/worker/authority-operations.ts`, `src/worker/authority.test.ts` |
| PSSR-D22 | Embed a published Site runtime-profile hint in SSR documents and let the browser resolver read it when env config is absent.                                      | Branch and production hosts can receive Worker SSR at `/` without using the `published-site.` hostname prefix; hydration must render the same shell.  | `src/worker/site-ssr.tsx`, `src/app/runtime-profile.ts`, `src/app/runtime-profile.test.ts`       |
...
### Published Site Cache Policy

- Successful SSR HTML: `Cache-Control: public, max-age=60, stale-while-revalidate=300`.
- Missing-page SSR HTML: `Cache-Control: public, max-age=30, stale-while-revalidate=60`.
- Error SSR HTML: `Cache-Control: no-store`.
- Public Site tree API reads: `Cache-Control: no-store`.
- SSR document responses vary on `Accept`.
- Tradeoff: published HTML can stay stale for a short TTL instead of using active purge in the first slice. Newly created pages can leave missing-page documents stale for at most the shorter 404 TTL in ordinary HTTP caches.
...
| PSSR-04 | shipped | PSSR-03    | Worker routing, wrangler, tests | Cloudflare routing sends published documents to Worker while API routes and assets keep existing behavior.    |
| PSSR-05 | shipped | PSSR-04    | cache policy, tests             | Successful, not-found, and error SSR responses have explicit cache headers and documented freshness tradeoff. |
| PSSR-06 | shipped | PSSR-05    | browser smoke, PRD              | Published Site smoke confirms content, hydration, links, theme toggle, no page errors, and PRD evidence.      |
...
- PSSR-04: promote Worker-first published document routing and static/client-shell route exclusions.
- `doc/current.md`: note published SSR cache policy and status behavior.
- PSSR-05: promote exact published SSR HTML cache headers and public tree no-store behavior.
...
- 2026-05-13: PSSR-04 shipped. Added `src/worker/routing.ts` and `src/worker/routing.test.ts` for Worker document route classification. `wrangler.jsonc` now binds `ASSETS` and uses selective Worker-first routing with `/*` plus negative exclusions for preview routes, generated app routes, dev/prod asset paths, and favicon. `src/worker/index.ts` can defer known client-shell and static asset requests to `env.ASSETS` when the Worker receives them.
- 2026-05-13: PSSR-04 tests updated in `src/worker/site-ssr.test.ts`, `src/site/public-site-ssr-characterization.test.tsx`, and `src/site/editing-publish-workflow.test.ts`. Coverage includes API requests staying API JSON, preview/admin/asset-like paths staying out of SSR, and Wrangler rejecting redundant positive patterns until the route list was reduced to the valid `/*` catch-all plus exclusions.
- 2026-05-13: PSSR-04 check evidence: restarted `devstate` after the Wrangler change; `.devstate/status.md` and `.devstate/status.json` reported checks ok, web ready at `https://34-public-site-ssr.formless.local`, and test watcher passing. `./tmp/devstate.json`, `./tmp/test.txt`, and `./tmp/check.txt` were absent. `.devstate/logs/service-test.txt` reported 50 files and 707 tests passing. `.devstate/logs/check-vite.txt` reported formatting complete and no warnings, lint errors, or type errors in 227 files.
- 2026-05-13: PSSR-04 browser smoke: `curl -sk -H 'Accept: text/html' https://34-public-site-ssr.formless.local/` returned initial HTML containing `<div id="app"><main` and the `formless.sitePageTree` payload with no loading shell. `bun browser --session pssr04` opened `/`, rendered David Peek, Code is magic, and Greetings, Robot content, returned no page errors, and after reload `bun browser network requests --filter /api/site/tree` reported no captured tree requests.
- 2026-05-13: PSSR-05 shipped. Added `src/worker/site-cache.ts` with short TTL published HTML policy, no-store error policy, and no-store public tree policy. `src/worker/site-ssr.tsx` now sets cache and `Vary: Accept` headers for success, not-found, and error documents, and converts failed tree reads into no-store error documents. `src/worker/authority-operations.ts` marks successful and missing public tree reads with explicit no-store headers.
- 2026-05-13: PSSR-05 tests updated in `src/worker/site-ssr.test.ts` and `src/worker/authority.test.ts`. Coverage includes successful SSR HTML cache headers, not-found SSR cache headers, no-store error SSR headers without leaking upstream error text, and public tree API no-store headers for successful and missing trees.
- 2026-05-13: PSSR-05 check evidence: final `devstate check` passed. `./tmp/devstate.json`, `./tmp/test.txt`, and `./tmp/check.txt` were absent in this checkout. `.devstate/status.md` and `.devstate/status.json` reported checks ok, web ready at `https://34-public-site-ssr.formless.local`, and test watcher passing. `.devstate/logs/service-test.txt` reported 3 files and 113 tests passing after the cache policy changes. `.devstate/logs/check-vite.txt` reported formatting complete and no warnings, lint errors, or type errors in 228 files.
- 2026-05-13: PSSR-05 browser smoke: `bun browser --session agent-browser --ignore-https-errors open https://34-public-site-ssr.formless.local/` opened the published root. `bun browser --session agent-browser get text body` showed David Peek, Code is magic, and Greetings, Robot content. `bun browser --session agent-browser errors` returned no page errors.
- 2026-05-13: PSSR-06 browser smoke first exposed a hydration mismatch on `https://34-public-site-ssr.formless.local/`: the Worker returned SSR Site HTML, but the browser resolved the branch host as the dev profile, React replaced `<main>` with the workbench shell, and the route landed on `/tasks`.
- 2026-05-13: PSSR-06 shipped the SSR runtime-profile handoff. `src/worker/site-ssr.tsx` now emits `<meta name="formless-runtime-profile" content="publishedSite" />`; `src/app/runtime-profile.ts` reads that document hint only when `VITE_FORMLESS_RUNTIME_PROFILE` is absent; tests cover the hint in `src/app/runtime-profile.test.ts` and `src/worker/site-ssr.test.ts`.
- 2026-05-13: PSSR-06 browser smoke: `bun browser --session pssr06 --ignore-https-errors` opened and reloaded `https://34-public-site-ssr.formless.local/`; DOM eval showed URL `/`, SSR app root `<main>`, no workbench/generated frame, header/footer present, no loading text, `formless.sitePageTree` slug `home`, and no captured `/api/site/tree/home` request before first paint.
- 2026-05-13: PSSR-06 browser smoke also toggled the public theme to `light`, verified `formless:public-site:theme = light` persisted across reload, clicked the published `/blog` link, rendered the Blog page with seeded post content, and `bun browser --session pssr06 errors` returned no page errors.
- 2026-05-13: PSSR-06 post-rebase adjustment: `src/worker/site-ssr.test.ts` now uses the rebased source Site post slug `/blog/agents-are-enablers` for the nested SSR document assertion.
- 2026-05-13: PSSR-06 final check evidence: after rebase and a clean `devstate stop`/`devstate start`, `.devstate/status.md` reported checks ok and services running. `./tmp/devstate.json`, `./tmp/test.txt`, and `./tmp/check.txt` were absent. `.devstate/logs/service-test.txt` reported 51 files and 726 tests passing. `.devstate/logs/check-vite.txt` reported formatting complete and no warnings, lint errors, or type errors in 230 files.
- 2026-05-13: PSSR-06 final browser smoke after the clean devstate restart repeated the published root checks, verified no initial `/api/site/tree/home` request, toggled and persisted the theme, clicked `/blog`, rendered the Blog page with existing post content, and returned no browser page errors.
- 2026-05-13: PSSR maintenance fixed production asset loading. Deployed `/projects` returned SSR HTML with `import RefreshRuntime from "/@react-refresh"` and `/src/main.tsx`, but no built CSS asset. `src/worker/site-ssr.tsx` now reads `/index.html` from `env.ASSETS`, extracts `/assets/*` script, modulepreload, and stylesheet tags, and falls back to dev tags only for local dev shells.
- 2026-05-13: PSSR maintenance evidence: `devstate check` first failed the new production-asset regression in `src/worker/site-ssr.test.ts`, then passed after the fix. Final `devstate check` reported checks ok, web ready at `https://formless.local`, and test watcher passing. `bun run build` produced `dist/client/index.html` with built `/assets/*` JS and CSS tags. Browser smoke `bun browser --session ssr-assets --ignore-https-errors ... https://formless.local/projects` rendered the Projects page with no browser errors.
