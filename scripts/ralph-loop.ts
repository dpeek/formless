...
function buildPrompt(prdPath: string): string {
  return [
    ...
    "6. Read `./tmp/state.txt` again and fix issues. Run relevant Bun checks. Use Browser Use only if app behavior changed.",
    "7. Rebase the current branch on local `main` before the final commit. Preserve your iteration changes with non-interactive git commands, and stop with `<blocked/>` on conflicts.",
    "8. Commit the chunk with a concise message. Do not amend existing commits.",
    "9. Final response must include changed files, checks, PRD status, and exactly one signal: `<task-done/>`, `<plan-done/>`, or `<blocked/>`.",
    "",
    "Loop contract:",
