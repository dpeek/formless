...
function buildPrompt(prdPath: string): string {
  return [
    ...
    "6. Read `./tmp/devstate.json`, `./tmp/test.txt`, and `./tmp/check.txt` again and fix issues. Do not run `vp test`, `vp check`, `bun test`, or `bun check` manually; `devstate start` owns those outputs.",
