import type {
  ...
  FormlessUiShellDestinationIdentity,
...
import { formatGeneratedWorkspaceCount } from "./formless-ui-workspace-projection.ts";
import {
...
export type GeneratedShellRootProjectionInput = {
  createSurfacesByQueryName?: Readonly<Record<string, FormlessUiCreateSurfaceContract | undefined>>;
...
export function selectGeneratedShellScope({
  ...
  routeWorld: RuntimeWorldMount | undefined;
  runtimeProfile: RuntimeProfile;
}): FormlessUiShellScope | undefined {
...
export function selectGeneratedShellRootSections({
  ...
  snapshot,
  today,
}: GeneratedShellRootProjectionInput): FormlessUiShellNavigationSectionContract[] {
...
export function selectGeneratedShellSyncStatus({
  ...
  status,
  worldLabel,
}: GeneratedShellSyncFacts): FormlessUiShellSyncStatusContract {
  return {
    details: [
...
export function selectGeneratedShellActiveDestination(
  sections: readonly FormlessUiShellNavigationSectionContract[],
): FormlessUiShellDestinationIdentity | null {
...
function instanceSection(currentPath: string): FormlessUiShellNavigationSectionContract {
  const path = normalizeRuntimeBrowserPath(currentPath);
  const destinations = [
    {
      href: runtimeTopologyRoutes.instanceRootRoute,
