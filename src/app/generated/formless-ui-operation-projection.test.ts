...
describe("generated Formless UI operation projection", () => {
  it("projects explicit button presentation, shared pending state, target count, and progress", () => {
    ...
    const sharedControl = projectGeneratedOperationFormlessUiControl({
  ...
  it("projects controlled destructive confirmation and committed, replayed, and failed feedback", () => {
    ...
    const pending = projectGeneratedOperationFormlessUiControl({
      binding,
    ...
    const failed = projectGeneratedOperationFormlessUiControl({
      binding,
      confirmationOpen: true,
      presentation: deletePresentation,
      state: {
    ...
    const committed = projectGeneratedOperationFormlessUiControl({
      binding,
      confirmationOpen: false,
      presentation: deletePresentation,
      state: completedState(binding.executionKey, "committed", 3_000),
    });
    const replayed = projectGeneratedOperationFormlessUiControl({
      binding,
      confirmationOpen: false,
      presentation: deletePresentation,
  ...
  it("projects affected counts and explicit feedback copy from normalized results", () => {
    ...
    expect(projectGeneratedOperationFormlessUiFeedback(binding, committedState)).toMatchObject({
      detail: "3 affected.",
      status: "committed",
      title: "Clear completed synced.",
    });
    expect(
      projectGeneratedOperationFormlessUiFeedback(binding, committedState, {
        copy: {
          committed: {
