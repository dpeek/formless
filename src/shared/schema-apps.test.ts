...
describe("schema app definitions", () => {
  it("declares the first route-backed schema apps in order", () => {
    expect(schemaApps.map((app) => app.key)).toEqual([
      "tasks",
      "estii",
      "site",
      "crm",
    ...
    expect(schemaApps.map((app) => app.route)).toEqual([
      "/tasks",
      "/estii",
      "/site",
      "/crm",
  ...
  it("looks up app definitions by schema key and route", () => {
    ...
    expect(findSchemaAppDefinition("cleartrace")?.label).toBe("ClearTrace");
    ...
    expect(findSchemaAppDefinitionByRoute("/cleartrace")?.key).toBe("cleartrace");
    expect(findSchemaAppDefinitionByRoute("/cleartrace/orders")?.key).toBe("cleartrace");
    expect(findSchemaAppDefinitionByRoute("/cleartrace/schema")?.key).toBe("cleartrace");
