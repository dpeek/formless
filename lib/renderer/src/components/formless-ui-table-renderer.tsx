...
import type {
  ...
  FormlessUiTableEditDialogContract,
  FormlessUiTableFooterCellContract,
...
type AstryxTableButtonContract = FormlessUiButtonContract | FormlessUiOperationButtonContract;
...
function AstryxTableRowWarningIndicator({
  ...
  warnings: FormlessUiTableContract["rows"][number]["warnings"];
}) {
...
function AstryxTableCellContent({
  ...
  contentRole: FormlessUiTableColumnContract["contentRole"];
  contextId: string;
  onFieldIntent: AstryxTableFieldIntentHandler;
  onOperationIntent: AstryxTableOperationIntentHandler;
  onTableIntent: FormlessUiTableIntentHandler;
...
function AstryxTableDisplayValue({ value }: { value: FormlessUiTableDisplayValueContract }) {
  return (
...
function AstryxTableValueStatus({ status }: { status: FormlessUiTableValueStatus }) {
...
function AstryxTableFooterCell({ cell }: { cell: FormlessUiTableFooterCellContract }) {
...
export function astryxTableDensity(density: FormlessUiTableContract["density"]): TableDensity {
  return density === "compact" ? "compact" : "spacious";
}

export function astryxTableSecondaryActionItems(
  actions: readonly FormlessUiTableActionContract[],
  onOperationIntent: AstryxTableOperationIntentHandler,
  onTableIntent: FormlessUiTableIntentHandler,
): DropdownMenuOption[] {
...
export function astryxTableButtonVariant(
...
function astryxTableRowHeaderCellId(
  table: FormlessUiTableContract,
  row: FormlessUiTableContract["rows"][number],
) {
...
function astryxTableCellStyle(
  column: FormlessUiTableColumnContract,
...
function astryxTableActionButton(action: FormlessUiTableActionContract) {
...
function astryxTableOrderingActionDisabled(
  action: FormlessUiTableOrderingContract["actions"][number],
) {
  return Boolean(action.disabled || action.pending?.isPending);
}

function astryxTableActionId(action: FormlessUiTableActionContract) {
...
function requiredAstryxTableCell(
  cells: FormlessUiTableContract["rows"][number]["cells"],
  columnId: string,
) {
