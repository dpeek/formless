...
} from "./formless-ui-list-renderer.tsx";
...
describe("Astryx list renderer", () => {
  ...
  it("dispatches field, primary, overflow, and ordering intents without exposing unavailable moves", () => {
    ...
    const listIntents: FormlessUiListIntent[] = [];
    const fieldIntent = {
      fieldName: "title",
  ...
  it("retains pending ordering status in overflow while suppressing duplicate dispatch", () => {
    const intents: FormlessUiListIntent[] = [];
...
function renderList(list: FormlessUiListContract) {
...
function readOnlyList(): FormlessUiListContract {
  const list = listContract({ pending: false });
  const item = list.items[0]!;
...
function listContract({ pending }: { pending: boolean }): FormlessUiListContract {
