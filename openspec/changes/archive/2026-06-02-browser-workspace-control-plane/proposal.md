...
- Remove runtime deployment execution history entities from reviewable workspace/control-plane source. Deployment attempts, evidence summaries, drift reports, and cleanup audit summaries are returned as display-safe gateway operation or deployment runtime status data instead of schema-owned source records.
- Remove `formless onboard` from the CLI surface. `formless dev` starts the local workspace runtime and the browser owns workspace initialization.
...
- `instance-control-plane`: App install, unified route, and deployment intent records become the canonical workspace and runtime source for browser onboarding, while deployment execution history leaves schema-owned source.
...
- `deployment-runtime`: Local deploy plan/apply can be driven through the workspace gateway while preserving credential, runner, attempt, evidence, drift, and cleanup-history boundaries.
...
- Affected APIs: new local-only workspace operation API; `formless onboard` is removed, while other workspace CLI command names keep their product role with new source semantics.
