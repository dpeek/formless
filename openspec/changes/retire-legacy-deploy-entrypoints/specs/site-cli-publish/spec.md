...
#### Scenario: Removed local Site publish surfaces

- **GIVEN** a user or browser attempts standalone Site publish through retained
  project publish wrappers, local publish broker routes, or local Site publish
  controls
- **WHEN** the request is parsed or invoked
- **THEN** the request fails before Cloudflare, Alchemy, Authority, filesystem,
  or provider mutation
- **AND** output directs the user to `formless deploy` for workspace deployment
  or `formless archive import-site` for standalone Site project migration
...
- **GIVEN** workspace route records create DNS, custom-domain, or redirect
  desired resources
- **WHEN** a user, browser client, or Worker route attempts a domain-specific
  provider apply command, control, helper, or apply-job mutation
...
#### Scenario: Domain apply jobs removed

- **GIVEN** route-derived domain intent exists
- **WHEN** a browser client or API caller attempts to create or complete a
...
- **AND** output directs the caller to workspace deploy
