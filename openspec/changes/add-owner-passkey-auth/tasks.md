...
- [x] 7.3 Run browser smoke with `bun browser ...` for first-owner setup, passkey login, app write access, and logout; mapped-host non-auth behavior is covered by worker routing tests and manual owner verification.
- [x] 7.4 Record evidence, decisions, blockers, and promotion notes in this OpenSpec change.
- [x] 7.5 During finalization, promote shipped instance-auth, authority-storage, and runtime-topology facts into `openspec/specs/`; do not archive the change.

Evidence:

- Files changed: `openspec/specs/instance-auth/spec.md`, `openspec/specs/authority-storage/spec.md`, `openspec/specs/runtime-topology/spec.md`, `openspec/changes/add-owner-passkey-auth/tasks.md`.
- Pre-existing red status: initial `devstate start` in this worktree failed because `add-owner-passkey-auth.formless.local` was still registered by sibling worktree process PID 14558. Stopped sibling detached worktree devstate, ran this worktree `devstate stop`, then ran `PORTLESS=0 devstate start`; `./.devstate/status.md` showed checks ok, web ready, and test watcher pass at 2026-06-01T05:39:16.719Z.
- Check: `PORTLESS=0 devstate check` passed; final `./.devstate/status.md` read shows checks ok, web ready, and test watcher pass at 2026-06-01T05:41:06.835Z.
- Finalization rebase: `git rebase main` completed on 2026-06-01. Structural conflicts were resolved in `src/worker/authority.ts`, `src/worker/default-app-installs.ts`, `src/worker/instance-app-installs-state.ts`, `src/worker/owner-passkeys.ts`, and `openspec/specs/runtime-topology/spec.md`; no semantic blocker remained.
- Rebase reconciliation: preserved `main` upgrade-status dispatch, control-plane app-install backfill, package revision/hash columns, and schema-owned app-route spec facts while keeping passkey ceremony dispatch, transaction-local first-owner default Site install creation, and mapped-host non-RP auth boundaries.
- Rebase check fixes: `src/worker/upgrade-status-api.ts` now reads backfilled control-plane installs for instance upgrade status, `src/worker/owner-passkeys.test.ts` uses a real Authority-backed harness for control-plane and installed-app storage routing, and `src/shared/upgrade-migrations.ts` records the rebased Site source schema hash.
- Finalization check: `devstate check` passed after rebase fixes; `./.devstate/status.md` shows checks ok, web ready, and test watcher pass at 2026-06-01T05:49:43.202Z.
- Finalization promotion: shipped `instance-auth` facts were promoted into `openspec/specs/instance-auth/spec.md`; authority storage passkey-session facts were promoted into `openspec/specs/authority-storage/spec.md`; canonical origin and mapped-host auth boundary facts were promoted into `openspec/specs/runtime-topology/spec.md`.
- Browser smoke: not rerun in this promotion-only session; existing section 7 first-owner setup, passkey login, app write access, logout, and mapped-host evidence remains below.
...
- Mapped-host browser handoff: created mapped `app` and `publicSite` domain mappings during browser-context smoke, then `bun browser --session igor-owner-passkey-section7-mapped-app --args "--host-resolver-rules=MAP mapped-app.example.test 127.0.0.1" open http://mapped-app.example.test:4976/login` reached the Vite dev server but stopped before Worker routing with `Blocked request. This host ("mapped-app.example.test") is not allowed.` Restarting devstate with `__VITE_ADDITIONAL_SERVER_ALLOWED_HOSTS=mapped-app.example.test,mapped-site.example.test` and with the single host `mapped-app.example.test` did not change that behavior in this dev stack. Owner accepted manual mapped-host browser verification outside the agent on 2026-06-01.
- Unblock decision: canonical browser smoke plus committed mapped-host worker routing tests cover the automated evidence for this change; mapped-host browser verification is manually owned, so no implementation blocker remains before finalization.
- Archive decision: change was not archived; archiving remains separate after review and merge.
