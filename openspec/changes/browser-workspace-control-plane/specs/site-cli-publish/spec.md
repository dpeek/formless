...
The CLI SHALL start or initialize a local Formless workspace before any
Cloudflare account or deployment mutation, while allowing browser onboarding to
own workspace initialization and first app install.

#### Scenario: Initialize local workspace

- **WHEN** a user runs `formless onboard`
- **THEN** the command starts the same local workspace initialization behavior
  exposed to the browser and writes a layout-only manifest version `1`
  `formless.json`
- **AND** the manifest has no target, app, route, domain, deploy, or default app
  policy intent
- **AND** configured reviewable record source, archive, media, and ignored local
  state roots are created
- **AND** it does not list Cloudflare accounts, deploy Cloudflare resources,
  create remote owner setup capability, open a remote setup URL, or write global
  instance state

#### Scenario: Explore locally after onboarding

- **WHEN** `formless dev` runs for a layout-only workspace
...
- **THEN** the local Authority records schema-owned `app-install` and
  `app-route` records and initializes install-scoped app state
...
to query, write, save, and compare schema-owned `app-install`, `app-route`,
`domain-mapping`, `redirect-intent`, and deployment records.
...
- **THEN** they read allowed `app-install`, `app-route`, `domain-mapping`,
  `redirect-intent`, `deploy-target`, `provider-config-ref`, and
  `deploy-desired-resource` records through the instance control-plane protocol
  or workspace record source
