...
The system SHALL treat `formless.json` and workspace archives as the reviewable
local source of truth for local-first Formless workspaces.
...
- THEN the system writes deterministic app archive and control-plane workspace
  source from Authority-backed state
...
- THEN `formless.json` stores reviewable target, archive, deploy, local state,
  app, default app policy, and domain intent
- AND a newly onboarded workspace stores no target, no declared apps, and
