...
- Validate the completion gate: `bun check:packages`.
...
- The supervisor ran `bun check:setup` before this worker session.
...
9. Record changed files, `bun check:packages` evidence, browser smoke evidence when app behavior changed, blockers, and split guidance in structured commit metadata.
10. Run `bun check:packages`. Current clean command output is required for done evidence. If the completion gate is red, diagnose and fix reasonably actionable failures before deciding whether you are blocked. Do not run `vp test`, `vp check`, `bun test`, or `bun check` directly.
...
- A done signal requires the latest `bun check:packages` from this worker worktree to exit 0.
- If `bun check:packages` fails or times out, do not emit `<task-done/>` or `<plan-done/>`. Use its output to identify and fix failures when there is a reasonable path forward without user input.
- Treat clean `bun check:packages` as part of shipping the selected section. Do not ignore or merely relabel red checks.
- Stop with `<blocked/>` only when there is no reasonable way forward without user input or a product, architecture, storage, security, public API, or task-scope decision. Record the exact failing checks, the fix attempts made, and why further progress needs input. Do not mark the selected task section complete while the completion gate is red.
...
- After resolving conflicts, run `bun check:packages` and record conflict files plus resolution evidence.
